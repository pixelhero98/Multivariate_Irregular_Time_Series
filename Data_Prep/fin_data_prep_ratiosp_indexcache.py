...
def make_collate_level_and_firstdiff(
    ...
    # NOTE: batching by date is handled by the Sampler; collate just stacks.
    def collate(batch):
        xs = [_torch.as_tensor(b[0]) for b in batch]
        ys = [_normalize_y(b[1]) for b in batch]
        ms = [b[2] for b in batch]
        V = _torch.stack(xs, dim=0).unsqueeze(1)
...
class _IndexBackedDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        if self.regression:
            ...
            y_t = torch.tensor(int(y_vec[-1] > 0.0), dtype=torch.int64)  # simple example for classification
...
def _build_date_batches_from_pairs(order_pairs: np.ndarray,
    ...
    days = _normalize_to_day(end_times.astype('datetime64[ns]').astype(np.int64))
