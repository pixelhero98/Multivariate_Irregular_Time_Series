...
#
# Main differences vs. earlier versions:
#   1) Cache format defaults to DISK-BACKED MEMMAP shards (train_X.npy, etc.).
#      Loaders stream directly from disk with np.memmap, avoiding RAM blowups.
#   2) Two-pass window count + memmap preallocation when building splits.
#   3) Ratio split loader builds *index views* over the memmaps instead of
#      concatenating arrays.
#   4) Keeps legacy NPZ compatibility (will read if only NPZ exists), but memmap
#      is strongly recommended.
#
# You can still call it exactly the same way as your current code.
# No changes required to your usage snippet.

from dataclasses import dataclass, field
import os, json, math, warnings, gc, shutil


# ---------- Date-aware batching helpers (built-in) ----------
import numpy as _np
from math import ceil as _ceil
from torch.utils.data import Sampler as _Sampler, DataLoader

class _ListBatchSampler(_Sampler):
    """Simple batch sampler that yields precomputed lists of dataset indices."""
...
def _build_date_batches_for_dataset(ds, dates_per_batch, min_real_entities, keep_time_meta: str):
    ...
    uniq, starts = _np.unique(days_sorted, return_index=True)
...
class LabeledWindowWithMetaDataset(Dataset):
    def __init__(self,
        ...
        # DO NOT cast if dtype already float32 (memmap would copy); enforce float32 earlier
        self.X = X
        if regression:
            if Y.ndim == 2:
                Y = Y[..., None]
...
def prepare_stock_windows_and_cache_v2(
    ...
    # ---- 1) Figure out raw columns we would need if we download ----
    ...
    # ---- Fast path: load cached per-ticker features if available ----
    ...
    # (NEW) Optional: add an ENTITY_ID scalar feature, normalized to [0,1]
    ...
    # Recompute common columns after optional ENTITY_ID insertion
    ...
    if normalize_per_ticker:
        ...
        if len(Xva): Xva, Yva = norm_apply(Xva, Yva, IDva)
        if len(Xte): Xte, Yte = norm_apply(Xte, Yte, IDte)
    ...
    with open(os.path.join(mem_dir, 'manifest.json'), 'w') as f: json.dump(manifest, f, indent=2)
    ...
    with open(_meta_path(data_dir), 'w') as f: json.dump(meta, f, indent=2)
    with open(_norm_path(data_dir), 'w') as f: json.dump(norm_stats, f)

    # Also write a tiny "marker" NPZ so legacy code won't break if someone expects the file to exist.
    # It contains no arrays—just a pointer.
...
def _open_split_memmaps(mem_dir: str, prefix: str):
    ...
    if os.path.exists(ctx_p): CT = np.lib.format.open_memmap(ctx_p, mode='r')
    if os.path.exists(yt_p):  YT = np.lib.format.open_memmap(yt_p, mode='r')
    return X, Y, I, CT, YT

def _load_legacy_npz(data_dir: str):
    # This will load everything into RAM; kept only as a fallback.
...
class _ConcatIndexDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 assets: List[str],
                 regression: bool = True):
        # splits: list of (X, Y, IDs, CT, YT) memmaps
        self.splits = splits
        self.split_ids = np.asarray(split_ids, dtype=np.int16)  # map 0=train,1=val,2=test -> choose tuple
        self.order = order                                      # [N] of (global) rows specifying (split, local idx)
        self.assets = assets
        self.regression = regression

    def __len__(self): return self.order.shape[0]
    ...
    def __getitem__(self, i: int):
        ...
        if CT is not None: meta['ctx_times'] = CT[local_idx]
        if YT is not None: meta['y_times']  = YT[local_idx]
        return x, y, meta
...
# Prefer memmap cache
    if _memmap_exists(data_dir):
        ...
        for sid, name in enumerate(('train', 'val', 'test')):
            ...
            if CT is not None and CT.size:
                end_ctx = CT[:, -1] if CT.ndim == 2 else CT
            else:
                end_ctx = None
    ...
    # Built-in date-aware batching (memmap branch)
    ...
    if date_batching:
        ...
        train_dl = DataLoader(ds_tr, batch_sampler=_ListBatchSampler(batches_tr), pin_memory=pin_memory,
                              ...
                              collate_fn=(collate_fn or collate_keep_meta))
