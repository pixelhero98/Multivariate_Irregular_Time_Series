...
from torch.cuda.amp import autocast  # (unused)
...
def diffusion_loss(model: LLapDiT, scheduler, x0_lat_norm: torch.Tensor, t: torch.Tensor,
    ...
    """Plain MSE on v/eps; no SNR weighting, no self-conditioning inside the loss."""
...
lr_sched = make_warmup_cosine(optimizer, total_steps, warmup_frac=crypto_config.WARMUP_FRAC, base_lr=crypto_config.BASE_LR, min_lr=getattr(crypto_config, 'MIN_LR', 1e-6))
# AMP scaler removed for stability
    ...
    for xb, yb, meta in train_dl:
        ...
        try:
            ...
            loss = diffusion_loss(diff_model, scheduler, mu_norm, t,
                                  ...
                                  self_cond=False, self_cond_p=0.0, snr_clip=0.0)

            # Guard: skip step on non-finite loss
            ...
            print("[warn] CUDA OOM on batch; skipping")
            optimizer.zero_grad(set_to_none=True)
            continue
