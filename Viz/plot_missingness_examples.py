...
def _prepare_index_with_run_experiment(
    ...
    """Use dataset-specific run_experiment to ensure the requested (window, horizon) are respected.

    This mirrors how training scripts specify horizons and rebuilds the window index if needed.
    """
    ...
    horizon = int(
        horizon_override if horizon_override is not None else meta.get("horizon", meta.get("max_horizon", 0))
    )

    coverage_default = float(meta.get("min_coverage", meta.get("coverage", 0.0) or 0.0))
    if "noaa" in dataset or "isd" in dataset:
...
def _select_asset_slice(
    ...
    slice_end = int(start_indices.max() + window + max(horizon, 0))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--noaa-window",
        type=int,
        help="Context window (K) to enforce via run_experiment for NOAA ISD.",
    )
    parser.add_argument(
        "--noaa-horizon",
        type=int,
        help="Forecast horizon (H) to enforce via run_experiment for NOAA ISD.",
    )
    parser.add_argument(
        "--physio-window",
        type=int,
        help="Context window (K) to enforce via run_experiment for PhysioNet CinC.",
    )
    parser.add_argument(
        "--physio-horizon",
        type=int,
        help="Forecast horizon (H) to enforce via run_experiment for PhysioNet CinC.",
    )
    parser.add_argument(
        "--coverage-quantile",
        type=float,
...
def main() -> None:
    ...
    noaa_meta, noaa_window, noaa_horizon = _prepare_index_with_run_experiment(
        noaa_paths, noaa_meta, args.noaa_window, args.noaa_horizon
    )
    physio_meta, physio_window, physio_horizon = _prepare_index_with_run_experiment(
        physio_paths, physio_meta, args.physio_window, args.physio_horizon
    )
    ...
    physio_example = _select_asset_slice(
        ...
        min_block=args.min_block,
        max_points=args.max_points,
        window=physio_window,
        horizon=physio_horizon,
    )
