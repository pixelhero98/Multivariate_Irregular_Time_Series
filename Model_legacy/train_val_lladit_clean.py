...
from Model.cond_diffusion_utils import (EMA, set_torch, encode_z_norm,
...
# ---- Estimate global latent stats for z (global) ----
...
z_mean, z_std = compute_latent_stats(vae, train_dl, device)
...
def train_one_epoch(epoch: int):
    ...
    for xb, yb, meta in train_dl:
        ...
        x_t, eps_true = scheduler.q_sample(z_norm, t, noise)
        ...
        with autocast(enabled=(device.type == "cuda")):
            if idx_c.numel() > 0:
                loss_c = diffusion_loss(
                    diff_model, scheduler, z_norm[idx_c], t[idx_c],
            ...
            if idx_u.numel() > 0:
                loss_u = diffusion_loss(
                    diff_model, scheduler, z_norm[idx_u], t[idx_u],
...
if z_mean is not None:
    z_mean = z_mean.to(device); z_std = ckpt['z_std'].to(device)
else:
    print('Checkpoint has mu_mean/mu_std only; recomputing z stats for Option Z...')
    z_mean, z_std = compute_latent_stats(vae, train_dl, device)
...
def finetune_vae_decoder(
    ...
    def _run_epoch(loader, tag, train: bool):
        ...
        for xb, yb, meta in loader:
            ...
            Beff, Hcur, Z = y_true.size(0), y_true.size(1), z_mean.numel()
            ...
            with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                ...
                y_hat_gen = decode_latents_with_vae(
                    vae, x0_norm_gen.detach(), z_mean=z_mean, z_std=z_std, window_scale=s
                )
...
def evaluate_regression(
    diff_model, vae, dataloader, device, z_mean, z_std, config, ema=None,
    ...
    for xb, yb, meta in dataloader:
        ...
        for _ in range(num_samples):
            ...
            y_hat_sample = decode_latents_with_vae(
                vae, x0_norm, z_mean=z_mean, z_std=z_std, window_scale=s_inf
            )
...
if True:
    ...
    evaluate_regression(
        diff_model, vae, test_dl, device, z_mean, z_std,
        config=crypto_config,  # Pass config object
