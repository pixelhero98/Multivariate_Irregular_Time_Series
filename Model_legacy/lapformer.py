...
class LaplaceSandwichBlock(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_heads: int, k: int, dropout: float = 0.0, attn_dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, x_time: torch.Tensor, pos_emb: torch.Tensor, t_vec: torch.Tensor, attn_bias: Optional[torch.Tensor], summary_kv_H: Optional[torch.Tensor] = None, sc_add_H: Optional[torch.Tensor] = None, dt: Optional[torch.Tensor] = None) -> torch.Tensor:
        z = self.analysis(x_time)                                 # [B,L,2k]
        h = self.lap2hid(z) + pos_emb + t_vec.unsqueeze(1)        # [B,L,H]
        if sc_add_H is not None: h = h + sc_add_H
        h = self.self_blk(h, attn_bias=None)
...
class LapFormer(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int, num_heads: int, laplace_k: Union[int, List[int]] = 8, dropout: float = 0.0, attn_dropout: float = 0.0, self_conditioning: bool = False):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([LaplaceSandwichBlock(input_dim, hidden_dim, num_heads, k, dropout, attn_dropout) for k in ks])
    ...
    def forward(self, x_tokens: torch.Tensor, t_vec: torch.Tensor, cond_summary: Optional[torch.Tensor] = None, sc_feat: Optional[torch.Tensor] = None, dt: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            h_time = blk(h_time, pos, t_vec, attn_bias=None, summary_kv_H=kvs[i], sc_add_H=sc_add_H)
