...
#             ctx_diff=series_diff,  # if you precompute diff, pass it here
...
#             cond_summary=None,
#             entity_ids=None,
#             y_obs: torch.Tensor = None,  # [B,L,D] observed values
#             obs_mask: torch.Tensor = None,  # [B,L] (1=observed, 0=free)
#             dt: Optional[torch.Tensor] = None,
#             series_dt: Optional[torch.Tensor] = None,
#             series_diff: Optional[torch.Tensor] = None,
...
#     ):
#         """
#         DDIM/Karras sampling with (optional) inpainting and self-conditioning
#         """
#         device = next(self.parameters()).device
#         B, L, D = shape
...
#             if not cfg_rescale:
#                 return guided
#             # rescale to match conditional branch stats (per sample)
#             reduce_dims = (1, 2)
...
#         # ---------- init ----------
#         x_t = torch.randn(B, L, D, device=device)
#         sc_feat_next = torch.zeros_like(x_t) if self_cond else None  # << cleaned init

#         if (series is not None) and (cond_summary is None):
#             cond_summary = self.context(series, pad_mask=None, dt=series_dt, entity_mask=series_mask)[0]  # [B,S,H]

#         T = int(self.scheduler.timesteps)
#         step_indices = _karras_step_indices(T, int(steps))
#         ts_prev = torch.cat([step_indices[1:], step_indices.new_tensor([-1])])

#         # ----- observation mask (expand once) -----
#         obs = obs_mask if obs_mask is not None else None
#         if obs is not None:
#             obs = obs.to(device=device, dtype=x_t.dtype)  # [B,L]
#             obs_u = obs.unsqueeze(-1)  # [B,L,1]
#             tar_scale = (1.0 - obs).unsqueeze(-1)  # [B,L,1] (weaker CFG on observed)
#         else:
#             obs_u = None
#             tar_scale = None

#         # project x_T to honour observations at first time
#         if (y_obs is not None) and (obs_u is not None):
#             t0_b = step_indices[0].repeat(B)
#             x_T_obs, _ = self.scheduler.q_sample(y_obs.to(device=device, dtype=x_t.dtype), t0_b)
#             x_t = obs_u * x_T_obs + (1.0 - obs_u) * x_t
...
#             pred_u = self.forward(
#                 x_t, t_b, series=None, series_mask=None, cond_summary=None,
#                 sc_feat=sc_feat_next if self_cond else None,
#                 entity_ids=entity_ids, dt=dt, series_dt=series_dt, series_diff=series_diff
#             )
#             pred_c = self.forward(
#                 x_t, t_b, series=series, series_mask=series_mask, cond_summary=cond_summary,
#                 sc_feat=sc_feat_next if self_cond else None,
#                 entity_ids=entity_ids, dt=dt, series_dt=series_dt, series_diff=series_diff
#             )

#             # CFG schedule
#             ab = _alpha_bar(t_b)  # [B,1,1]
#             if isinstance(guidance_strength, (tuple, list)):
#                 g_min, g_max = guidance_strength
#                 g_scalar = g_min + (g_max - g_min) * (1.0 - ab) ** guidance_power
#             else:
...
#             # x0 estimate (respects predict_type)
#             x0_hat = self.scheduler.to_x0(x_t, t_b, pred, param_type=self.predict_type)
#             last_x0 = x0_hat
#             if self_cond:
...
#                     x_t, t_b, tprev_b, pred, param_type=self.predict_type, eta=eta,
#                 )
#             else:
#                 x_t = x0_hat  # final

#             # re-impose observations
#             if (y_obs is not None) and (obs_u is not None):
#                 if int(t_prev_i) >= 0:
#                     t_inpaint = t_prev_i.repeat(B)
#                     x_obs_t, _ = self.scheduler.q_sample(y_obs.to(device=device, dtype=x_t.dtype), t_inpaint)
