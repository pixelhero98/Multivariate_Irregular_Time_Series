...
def calculate_v_variance(scheduler, dataloader, vae, device, latent_stats,
    ...
    """Calculates the variance of the v-prediction target over a given dataloader for VAE2D."""
    all_v_targets = []
    ...
    for xb, yb, meta in dataloader:
        m_bn = meta["entity_mask"]
        mu_norm, _ = encode_mu_norm_2d(
            vae, yb, m_bn,
...
# ============================ Helpers for VAE2D (entity×time) ============================
def batch_ids_from_mask(mask_bn: torch.Tensor, device: torch.device) -> torch.Tensor:
    """mask_bn: [B,N] bool -> batch_ids: [Beff] mapping rows to original batch index"""
    B, N = mask_bn.shape
    ids = torch.arange(B, device=device).unsqueeze(1).expand(B, N).reshape(B * N)
    flat = mask_bn.reshape(B * N)
    return ids[flat]


@torch.no_grad()
def compute_latent_stats_2d(vae2d, dataloader, device, use_ewma: bool, ewma_lambda: float,
                            clip_val: float = 5.0):
    """Estimate global mean/std for two-stage normalization using VAE2D.
    Returns (mu_mean, mu_std) over flattened (entities kept) × H' × C."""
    all_mu_w = []
    for (_, yb, meta) in dataloader:
        m_bn = meta["entity_mask"].to(device)          # [B,N] bool
        y = yb.to(device)                               # [B,N,H]
        x_in = y.unsqueeze(-1)                          # [B,N,H,1]
        m_full = m_bn.unsqueeze(-1).expand_as(y)        # [B,N,H]
        # encode
        _, mu_g, _logv_g, _z_grid, _cov, _grid, _orig = vae2d(x_in, mask=m_full)
        B, Np, Hp, C = mu_g.shape  # with p_n=1 -> Np = N
        # select valid entities and flatten to [Beff, H', C]
        mu_flat = mu_g.reshape(B * Np, Hp, C)
        keep = m_bn.reshape(B * Np)
        if not keep.any():
            continue
        mu_sel = mu_flat[keep]
        # window scaling
        if use_ewma:
            s = ewma_std(mu_sel, lam=ewma_lambda)  # [Beff,1,C]
        else:
            s = mu_sel.std(dim=1, keepdim=True, correction=0).clamp_min(1e-6)
        mu_w = (mu_sel / s).clamp(-clip_val, clip_val)
        all_mu_w.append(mu_w.detach().cpu())

    if len(all_mu_w) == 0:
        raise RuntimeError("No valid data for latent stats. Check your masks/dataloader.")
    mu_cat = torch.cat(all_mu_w, dim=0)
    mu_mean = mu_cat.mean(dim=(0, 1)).to(device)
    mu_std  = mu_cat.std (dim=(0, 1), correction=0).clamp_min(1e-6).to(device)
    return mu_mean, mu_std
...
@torch.no_grad()
def encode_mu_norm_2d(vae2d, yb: torch.Tensor, mask_bn: torch.Tensor, *,
                      use_ewma: bool, ewma_lambda: float,
                      mu_mean: torch.Tensor, mu_std: torch.Tensor):
    """Encode with VAE2D then two-stage normalize.
    Inputs:
      - yb: [B,N,H] (ground-truth target window)
      - mask_bn: [B,N] bool (valid entities)
    Returns:
      - mu_norm: [Beff, H', C]
      - batch_ids: [Beff] mapping back to batch rows (to align conditioning)"""
    device = mu_mean.device
    x_in = yb.to(device).unsqueeze(-1)                   # [B,N,H,1]
    m_bn = mask_bn.to(device)                            # [B,N]
    m_full = m_bn.unsqueeze(-1).expand_as(yb)            # [B,N,H]
    _, mu_g, _logv_g, _z_grid, _cov, _grid, _orig = vae2d(x_in, mask=m_full)
    B, Np, Hp, C = mu_g.shape
    mu_flat = mu_g.reshape(B * Np, Hp, C)
    keep = m_bn.reshape(B * Np)
    if not keep.any():
        return None, None
    mu_sel = mu_flat[keep]                                # [Beff,H',C]
    ...
    if use_ewma:
        ...
        s = mu_sel.std(dim=1, keepdim=True, correction=0).clamp_min(1e-6)
    mu_w = (mu_sel / s).clamp(-5.0, 5.0)

    mu_mean = mu_mean.to(device=device, dtype=mu_sel.dtype)
    mu_std  = mu_std.to (device=device, dtype=mu_sel.dtype).clamp_min(1e-6)
    mu_norm = ((mu_w - mu_mean) / mu_std).clamp(-5.0, 5.0)
    mu_norm = torch.nan_to_num(mu_norm, nan=0.0, posinf=0.0, neginf=0.0)

    batch_ids = batch_ids_from_mask(m_bn, device=device)  # [Beff]
    return mu_norm, batch_ids


@torch.no_grad()
def decode_latents_with_vae_2d(vae2d, x0_norm: torch.Tensor,
                               mu_mean: torch.Tensor, mu_std: torch.Tensor,
                               window_scale: Optional[torch.Tensor] = None) -> torch.Tensor:
    """Decode Beff sequences produced by diffusion with VAE2D's decoder (no encoder memory).
      - x0_norm: [Beff, H', C]
    Returns:
      - y_hat: [Beff, H, 1] where H = H' * p_h (cropped if needed)"""
    mu_mean = mu_mean.to(device=x0_norm.device, dtype=x0_norm.dtype)
    mu_std  = mu_std.to (device=x0_norm.device, dtype=x0_norm.dtype).clamp_min(1e-6)
    mu_w = x0_norm * mu_std.view(1, 1, -1) + mu_mean.view(1, 1, -1)
    s = 1.0 if window_scale is None else window_scale  # scalar or [Beff,1,C]
    mu_est = mu_w * s

    Beff, Hp, C = mu_est.shape
    # Prepare tokens for decoder
    dec_in = vae2d.to_dec(mu_est)                       # [Beff,H',d_model]
    dec = vae2d.decoder(dec_in, memory=None, self_pad=None, mem_pad=None)  # self-attn only
    out_tokens = vae2d.out_proj(dec)                    # [Beff,H',P] with P=pn*ph
    # Unpatch back; N'=1 for per-entity streams
    grid = (1, Hp)
    H_orig = Hp * vae2d.ph
    y_hat = vae2d.unpatch(out_tokens, grid, (1, H_orig))  # [Beff,1,H,1]
    return y_hat.squeeze(1)                               # [Beff,H,1]
